...
package uk.co.nickthecoder.familyalbum.bean;
...
import uk.co.nickthecoder.familyalbum.action.GetPostAction;
import uk.co.nickthecoder.familyalbum.model.Album;
import uk.co.nickthecoder.familyalbum.model.Family;
import uk.co.nickthecoder.familyalbum.model.FamilyAlbum;
import uk.co.nickthecoder.familyalbum.model.Photo;
import uk.co.nickthecoder.familyalbum.model.RelationshipType;
import uk.co.nickthecoder.familyalbum.model.SipAgeComparator;
import uk.co.nickthecoder.familyalbum.model.Subject;
import uk.co.nickthecoder.familyalbum.model.SubjectInPhoto;
import uk.co.nickthecoder.familyalbum.model.SubjectType;
  ...
  */
    ...
    private Family getFamily(Integer familyId)
    {
        try {
            ...
            // System.out.println( "Failed to find family : " + familyId +
            // " Error : " + e );
            return null;
        }
    }
    ...
    public Subject getSubject()
    {
        if (_subject == null) {
            ...
            try {
                ...
                // System.out.println( "Failed to get subject : " +
                // getSubjectId() );
                return null;
            }
    ...
    private Integer guessPhotoId()
    {
        if (_photoId == null) {
            ...
            if (getSubjectInPhoto().getPhoto() == null) {
                ...
                // System.out.println( "guessed p " +
                // getSubjectInPhoto().getPhoto().getPhotoId() );
    ...
    private Integer guessSubjectInPhotoId()
    {
        if (_subjectInPhotoId == null) {
            if (getSubject().getDefaultSubjectInPhoto() == null) {
                ...
                // System.out.println("Guessed sip : " +
                // getSubject().getDefaultSubjectInPhoto().getSubjectInPhotoId()
                // );
        ...
        if (_subjectInPhoto == null) {
            ...
            try {
                ...
                if (subjectInPhotoId == null) {
                    ...
                    try {
                        _subjectInPhoto = (SubjectInPhoto) HibernateUtil.currentSession().get(SubjectInPhoto.class,
                                        subjectInPhotoId);
                    } catch (Exception e) {
                        // System.out.println( "inner try failed" );
                    }

                    // if not found, then try to use the default one from
                    // subject
                    if (_subjectInPhoto == null) {
    ...
    public List<Subject> getSubjects()
    {
        ...
        if (getSubjectTypeId() == null) {
            ...
            List<Subject> result = HibernateUtil.currentSession()
                            .createQuery("from Subject where familyId=:familyId order by name2, name1")
                            .setParameter("familyId", familyId).list();
            return result;

        } else {

            @SuppressWarnings("unchecked")
            List<Subject> result = HibernateUtil
                            .currentSession()
                            .createQuery("from Subject where familyId=:familyId and subjectTypeId=:subjectTypeId order by name2, name1")
                            .setParameter("familyId", familyId)
                            .setParameter("subjectTypeId", getSubjectType().getSubjectTypeId()).list();
    ...
    {
        @SuppressWarnings("unchecked")
        List<Album> result = HibernateUtil.currentSession()
                        .createQuery("from Album where familyId=:familyId order by label")
                        .setParameter("familyId", getFamily().getFamilyId()).list();
    ...
    {
        @SuppressWarnings("unchecked")
        List<Family> result = HibernateUtil.currentSession().createQuery("from Family order by label").list();
    ...
    {
        @SuppressWarnings("unchecked")
        List<RelationshipType> result = HibernateUtil.currentSession().createQuery("from RelationshipType").list();
    ...
    public SipAgeComparator getSipAgeComparator()
    {
